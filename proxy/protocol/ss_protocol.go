...
func AddressEncoding(req sproxy.IRequest) ([]byte, error) {
	var atyp uint8 = proxyM.AddrTypeDomain
	ip := net.ParseIP(req.Domain())
	addr := []byte(req.Domain())
	if ip != nil {
		if len(ip) == net.IPv4len {
			atyp = proxyM.AddrTypeIPv4
		} else {
			atyp = proxyM.AddrTypeIPv6
		}
		addr = []byte(ip)
	}
	if len(addr) == 0 {
	...
	switch atyp {
	case proxyM.AddrTypeIPv4, proxyM.AddrTypeIPv6:
		buffer.WriteByte(atyp)
		buffer.Write(addr)
		buffer.Write(portBytes)
	case proxyM.AddrTypeDomain:
		buffer.WriteByte(atyp)
