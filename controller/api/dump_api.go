...
func SetAllowDump(ctx *gin.Context) {
	...
	switch allow_dump {
	case "true":
		proxyM.SetAllowDump(true)
	case "false":
		proxyM.SetAllowDump(false)
	case "":
	default:
		response.Code = 1
	...
	switch allow_mitm {
	case "true":
		proxyM.SetAllowMitm(true)
	case "false":
		proxyM.SetAllowMitm(false)
	case "":
	default:
		response.Code = 1
...
func DumpRequest(ctx *gin.Context) {
	...
	if r.Status != proxyM.RecordStatusCompleted {
		response.Code = 1
	...
	if reqSize > proxyM.LargeRequestBody {
	...
	if respSize > proxyM.LargeResponseBody {
		buffer.WriteString("large body")
		resp.Body.Close()
	} else {
		var r io.Reader
		if resp.Header.Get("Content-Encoding") == "gzip" {
			...
			if err != nil {
				log.Logger.Errorf("[Proxy-Controller] [%d] gzip init for response failed: %v", id, err)
				response.Code = 1
				response.Message = err.Error()
				ctx.JSON(500, response)
				return
			...
			if err != nil {
				log.Logger.Errorf("[Proxy-Controller] [%d] deflate init for response failed: %v", id, err)
				response.Code = 1
				response.Message = err.Error()
				ctx.JSON(500, response)
				return
