...
import (
	...
	"github.com/demonoid81/proxyM/controller"
	...
	_ "github.com/demonoid81/proxyM/ciphers"
	_ "github.com/demonoid81/proxyM/proxy/protocol"
	_ "github.com/demonoid81/proxyM/proxy/selector"
)
...
func main() {
	configPath := flag.String("c", "proxy.yaml", "configuration file path")
	...
	<-signalChan
	log.Logger.Info("[Proxy] is shutdown, see you later!")
...
func loadConfig(configPath string) (conf *config.Config, err error) {
	...
	//init HttpMap
	if err = proxyM.ApplyHTTPModifyConfig(conf); err != nil {
		return
	}
	//init MITM
	if err = proxyM.ApplyMITMConfig(conf); err != nil {
		return
	}
	return
}
