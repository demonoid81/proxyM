import { asyncRoutes, constantRoutes, syncRoutes } from '@/router/routes'
...
const actions = {
  getUserMenutree: async ({ commit }) => {
    ...
    // console.log(syncRoutes)
      // const { list } = syncRoutes
    let _asyncRoutes = syncRoutes
      // const _asyncRoutes = [],
       let  permissions = []
      ...
      // 根据获取的数据生成各模块的权限编码
      generaPermissions(permissions, _asyncRoutes)
      // 将后备路由添加到动态路由后面
      // const __asyncRoutes = _asyncRoutes.concat(asyncRoutes)
    const __asyncRoutes = _asyncRoutes
