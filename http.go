...
import (
	...
	rule2 "github.com/demonoid81/proxyM/rule"
...
func HandleHTTP(co net.Conn) {
	...
	if err != nil {
		log.Logger.Errorf("[HTTP] proxyM.IConn wrap net.Conn failed: %v", err)
		return
	}
	log.Logger.Debugf("[HTTP] [ID:%d] proxyM.IConn wrap net.Conn success", conn.GetID())
